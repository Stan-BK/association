<template>
  <div class="announcement">
    ...
    <div class="announcement-content">
      <iframe ref="content" :srcdoc="announcement.content" frameborder="0" width="100%"></iframe>
    </div>
...
export default {
  ...
  computed: {
    isCollected() {
      return this.$store.state.user.announcement_collect?.split(',').includes(String(this.announcement.announcement_id))
    }
  },
  methods: {
    collectAnnouncement() {
      if (this.$store.state.user.user_id) {
        if (!this.collectDebounce) {
          ...
          this.$axios[`$${ this.isCollected ? 'delete' : 'put' }`]
            (`/api/announcement/collect?announcement_id=${ this.announcement.announcement_id }`)
            .then(res => {
              this.$message({
                type: this.isCollected ? 'info' : 'success',
                message: this.isCollected ? '取消收藏' : '收藏成功'
              })
              this.$store.commit("user/SET_announcement_collect", res)
